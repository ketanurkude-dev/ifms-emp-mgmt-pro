...
export default function Otp() {
  ...
  // Read once on mount so this doesn't change when we clear it on success
  // below (a plain sessionStorage.getItem() on every render would flip to
  // null right as we navigate away, re-triggering the redirect below).
  const [pendingToken] = useState(() => sessionStorage.getItem("pending_token"));

  // No login in progress -> send the user back to login.
  useEffect(() => {
    if (!pendingToken) {
      navigate("/login");
    }
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-6">
          ...
          <input
            ...
            maxLength={6}
            placeholder="000000"
            className="w-full border border-slate-300 rounded-md px-3.5 py-2.5 tracking-[0.5em] text-center text-xl font-medium text-slate-800 placeholder:tracking-normal placeholder:text-slate-300 focus:outline-none focus:ring-2 focus:ring-teal-600 focus:border-teal-600"
            value={otp}
