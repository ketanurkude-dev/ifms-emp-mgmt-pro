...
export default function AuditLog() {
  ...
  const [filters, setFilters] = useState({ entity_type: "", action: "", employee_id: "", result: "" });
    ...
    <AppLayout>
      <div className="bg-white border border-slate-200 rounded p-6">
        <div className="flex items-center justify-between mb-4">
          <h1 className="font-semibold text-slate-800">{t("navAuditLog")}</h1>
          <button onClick={() => downloadFile(`/audit/logs/export?${buildParams().toString()}`, "audit_log.csv")} className="text-xs text-teal-800 font-medium">
            Export CSV
          </button>
        </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          ...
          <input className={inputClass} placeholder="Employee ID" value={filters.employee_id} onChange={(e) => setFilters({ ...filters, employee_id: e.target.value })} />
          <select className={inputClass} value={filters.result} onChange={(e) => setFilters({ ...filters, result: e.target.value })}>
            <option value="">Any result</option>
            <option value="Success">Success</option>
            <option value="Failure">Failure</option>
          </select>
          <button onClick={handleSearch} className="bg-teal-800 text-white rounded-md px-4 py-1.5 text-sm font-medium hover:bg-teal-900">
            Search
          </button>
        </div>
