...
def _query_logs(
    db: Session, entity_type: str | None, action: str | None, employee_id: int | None,
...
def search_audit_logs(
    ...
    logs = _query_logs(db, entity_type, action, employee_id, result, date_from, date_to).limit(limit).all()
    log_action(db, employee_id=None, actor_id=approver.id, actor_role=approver.role, action="Audit log searched", entity_type="AuditLog", entity_id=None)
    db.commit()
    return logs
...
def export_audit_logs(
    ...
    logs = _query_logs(db, entity_type, action, employee_id, result, date_from, date_to).limit(5000).all()
    rows = [
        {
            "event_id": log.id, "timestamp": log.server_date, "actor_id": log.actor_id, "actor_role": log.actor_role,
            "employee_id": log.employee_id, "entity_type": log.entity_type, "entity_id": log.entity_id,
    ...
    log_action(db, employee_id=None, actor_id=approver.id, actor_role=approver.role, action="Audit log exported", entity_type="AuditLog", entity_id=None)
    db.commit()
